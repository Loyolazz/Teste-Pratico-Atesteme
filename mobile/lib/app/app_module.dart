...
class AppModule extends Module {
  ...
  void binds(Injector i) {
    ...
    i.addLazySingleton(() => OfflineSyncService(i<ApiClient>(), i<LocalDatabase>()));
    i.addLazySingleton(() => AuthRepository(i<ApiClient>()));
    i.addLazySingleton(() => AuthStore(i<AuthRepository>(), i<TokenStorage>(), i<LocalDatabase>()));
    i.addLazySingleton(() => ProjectRepository(i<ApiClient>(), i<LocalDatabase>(), i<OfflineSyncService>()));
    i.addLazySingleton(() => ProjectStore(i<ProjectRepository>()));
    i.addLazySingleton(() => TaskRepository(i<ApiClient>(), i<LocalDatabase>(), i<OfflineSyncService>()));
    i.addLazySingleton(() => TaskStore(i<TaskRepository>(), i<NotificationService>()));
  }
